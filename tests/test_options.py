...
from ..model_csv.columns import ColumnValidationError, MethodColumn, StaticColumn
...
class OptionsTest(TestCase):
    def setUp(self) -> None:
        self.columns = {
            ...
            "s_i_x": StaticColumn(
                index=1, read_value=False, write_value=False
            ),  # False, False
            "x_i_x": MethodColumn(
                index=2, read_value=False, write_value=False
            ),  # False, False
    ...
    def test_get_columns_is_static(self):
        opt = CsvOptions(meta=self.meta, columns=self.columns, parts=[])
        self.assertTrue(
            all([col.name.startswith("s_") for col in opt.get_columns(is_static=True)])
        )

        self.assertTrue(
            all([col.name.startswith("x_") for col in opt.get_columns(is_static=False)])
        )

    def test_get_columns_has_value(self):
        opt = CsvOptions(meta=self.meta, columns=self.columns, parts=[])
        self.assertTrue(
            all([col.name.endswith("_v") for col in opt.get_columns(read_value=True)])
        )

        self.assertTrue(
            all([col.name.endswith("_v") for col in opt.get_columns(write_value=True)])
        )

        self.assertTrue(
            all([col.name.endswith("_x") for col in opt.get_columns(read_value=False)])
        )

        self.assertTrue(
            all([col.name.endswith("_x") for col in opt.get_columns(write_value=False)])
        )
    ...
    def test_get_columns_read_index(self):
        ...
        self.assertTrue(
            all(
                [
                    "_i_" in col.name
                    for col in opt.get_columns(r_index=True, original=True)
                ]
            )
        )
        self.assertTrue(
            all(["_i_" in col.name for col in opt.get_columns(r_index=True)])
        )
        ...
        self.assertFalse(
            "s_x_v"
            in [col.name for col in opt.get_columns(r_index=True, original=True)]
        )

        self.assertTrue(
            "s_x_v"
            in [col.name for col in opt.get_columns(r_index=False, original=True)]
        )

        self.assertTrue("s_x_v" in [col.name for col in opt.get_columns(r_index=True)])

        self.assertFalse(
            "s_x_v" in [col.name for col in opt.get_columns(r_index=False)]
        )
    ...
    def test_get_columns_write_index(self):
        ...
        self.assertTrue(
            all(
                [
                    "_i_" in col.name
                    for col in opt.get_columns(w_index=True, original=True)
                ]
            )
        )
        self.assertTrue(
            all(["_i_" in col.name for col in opt.get_columns(w_index=True)])
        )
        ...
        self.assertFalse(
            "s_x_v"
            in [col.name for col in opt.get_columns(w_index=True, original=True)]
        )
        self.assertTrue(
            "s_x_v"
            in [col.name for col in opt.get_columns(w_index=False, original=True)]
        )

        self.assertTrue("s_x_v" in [col.name for col in opt.get_columns(w_index=True)])
        self.assertFalse(
            "s_x_v" in [col.name for col in opt.get_columns(w_index=False)]
        )
    ...
    def test_get_columns_for_read(self):
        ...
        self.assertSetEqual(
            {"s_i_v", "x_i_v"}, set(col.name for col in opt.get_columns(for_write=True))
        )
    ...
    def test_get_indexes(self):
        ...
        self.assertSetEqual(
            ...
            set(opt.get_r_indexes()),
        )
        self.assertSetEqual(
            {
                0,
            },
            set(opt.get_w_indexes()),
        )
